...
```
google-authenticator -s /config/.google_authenticator
```

Scan the QR code with your authenticator application and answer the prompts. Every login from that point on asks for a verification code.
...
* **29.07.26:** - Add optional OTP (2FA) support via `OTP_ACCESS`.
* **29.07.26:** - Add a healthcheck reporting whether sshd is listening.
